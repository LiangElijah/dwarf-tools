...
int dwarf_addr_cal(st_dieNode_t *entry, 
    st_str_t *strBuf, 
    st_addr_t *addrBuf) {
    if((entry == NULL) || (strBuf == NULL)) return AddrStatus_EMPTYPTR;
    ...
    for(int n = 0; n < strBuf->segmentNum; n++) {
        if(n == 0) {
                ...
                list_for_each_entry(cu_node, &entry->column, column) {
                    if(cu_node->row.next != NULL)
                    {
                        ...
                        if(strcmp((char *)var_node->un.var.name, strBuf->segment[n]) == 0) {
                            ...
                            if(found == 1) break;
                            else return AddrStatus_NOFOUND;
                        }
                    }
                }
            }
            ...
            if(var_node->row.next != NULL) {
                ...
                num = type_node->un.type.dimensionNum;
                deep = type_node->un.type.dimension;
                size = type_node->un.type.byte_size;
            } else return AddrStatus_NOTYPE;
        } else /*if(n > 0)*/ {
            found = 0;
            if(n == 1) {
                if((var_node->row.next != NULL) && (var_node->row.next->next != NULL))
                    mem_head = var_node->row.next->next;
                else return AddrStatus_NOMEM;
            } else {
            ...
            if(strcmp((char *)mem_node->un.mem.name, strBuf->segment[n]) == 0) {
                ...
                if(found == 0) return AddrStatus_NOFOUND;
            }
    ...
    if(mem_node != NULL) {
        ...
        if(mem_node->row.next != NULL) {
            ...
            if(type_node->un.type.name != NULL) {
                ...
                addrBuf->typName = type_node->un.type.name_typedef;
            }
        } else return AddrStatus_NOTYPE;
    } else {
        if(var_node->row.next != NULL) {
